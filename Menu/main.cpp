...
void cache()
{
	while (true) {
		...
		Uworld = read<DWORD_PTR>(sdk::module_base );
		...
		offests::uworld = read<uint64_t>(sdk::module_base );
...
void render() {
	...
	if (ImGui::Begin(("BlackSqaudKernalCheat"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar ))
...
int main() {
	...
	if (driver->Init(FALSE)) {
		printf(("Success!\n"));
		Sleep(1000);
		driver->Attach((L"BlackSquadGame.exe"));
		
			SetupWindow();

			DirectXInit(MyWnd);

			sdk::process_id = driver->GetProcessId((L"BlackSquadGame.exe"));

			sdk::module_base = driver->GetModuleBase((L"BlackSquadGame.exe"));
			//sdk::module_base = driver->GetProcessThreadNumByID((sdk::process_id));

			printf(("BlackSquad pid :0x%llX\n"), sdk::process_id);
			printf(("BlackSquad Base address:0x%llX\n"),   sdk::module_base );
